...
?>
		...
		<tr><td bgcolor=navy><FONT class=a1 color=#ffffff>&nbsp;<B><?php  print TEXT_OPERATORS ?></B></FONT></td>
		<tr><td class=WIN>
...
<br>	<input name="op_name" value="<?php echo $row['name'];?>"> Name
<br>	<input name="username" value="<?php echo $row['username'];?>"> Username
<br>	<input name="password" value="<?php echo $row['password'];?>"> Password
<br>	<input type="checkbox" name="can_delete" "<?php echo ($row['can_delete']?"CHECKED":"");?>"> Can delete
<br>	Details<br><textarea name="details" rows=6 cols=40><?php echo $row['details'];?></textarea>

<?php  if( $act=="change") {				?>
	<input type=hidden name="act" value="change_write">
	<input type=hidden name="id" value="<?php echo $row['operator_id'];?>">
<?php  }else{						?>
