...
<tr><td class=WIN>
	...
	<table>
	<tr>
		<td colspan=2><?php  print TEXT_SHOW_BOOKINGS ?>
		    <select name="b_type">
			    <option value='made'><?php  print TEXT_MADE ?>
			    <option value='reserved'><?php  print TEXT_RESERVED ?>
		    </select>
		</td>
	</tr>
	<tr>
	...
	<input type="submit" value=<?php  print TEXT_FILTER ?> ><input type="checkbox" name="show_all" <?php echo $show_all?"CHECKED":""; ?> ><?php  print TEXT_SEEN_BOOKINGS ?>
	</form>
</td>
<td>
...
<form>
  <input type="button" value=<?php  print TEXT_BY_NUMBER ?> onClick="zoom_number();">
</form>
<?php  print TEXT_RED_NUMBERS ?><br>
<?php  print TEXT_GRAY_NUMBERS ?>

<table border=1 cellpadding=4 width="100%">
<tr align="right">
	<th>ID</th>
	<th><?php  print TEXT_SPEC ?></th>
	<th><?php  print TEXT_CLIENT ?></th>
    <th><?php  print TEXT_TIME_BOOKINGS ?></th>
	<th><?php  print TEXT_ARRIVAL_DATE ?></th>
	<th><?php  print TEXT_DEPARTURE_DATE ?></th>
<!--

	<th><?php  print TEXT_SINGLE ?></th>
	<th><?php  print TEXT_TWIN ?></th>
	<th><?php  print TEXT_DOUBLE ?></th>
	<th><?php  print TEXT_TRIPLE ?></th>
	<th><?php  print TEXT_EXECUTIVE ?></th>
-->
    <th><?php  print TEXT_ACTION ?></th>
</tr>
<?php 	while( $row = mysql_fetch_array( $res ) )  {	?>
<tr align="right">
	<td><?php echo ($row['is_seen']?'<font color="#888888">':'<font color="#FFFFFF">').$row['booking_id'];?></font></td>
	<td><?php echo ( $row['special_id'] ? "Yes" : "&nbsp;" );?></td>
	<td><?php echo ($row['client_id']?$row['client_id']:("<font color=red>".$row['corporate_id']."</font>"));?></td>
    <td><?php echo $row['booking_time'];?></td>
	<td><?php echo local_date($row['start_date']);?></td>
	<td><?php echo local_date($row['end_date']);?></td>
<!--
	<td><?php echo $row['singles'];?></td>
	<td><?php echo $row['twices'];?></td>
	<td><?php echo $row['doubles'];?></td>
	<td><?php echo $row['triples'];?></td>
	<td><?php echo $row['executives'];?></td>
-->
    <td><a href="reservations_zoom.php?id=<?php echo $row['booking_id'];?>"><?php  print TEXT_VIEW ?></a></td>
</tr>
<?php 	}					?>
</table>
