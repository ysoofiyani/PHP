...
<tr><td bgcolor=navy><FONT class=a1 color=#ffffff>&nbsp;<B><?php  print TEXT_PAGES ?></B></FONT></td>
...
if($file>0 AND !$act)
{
	...
	while( $row = mysql_fetch_array( $res ) ) 
	  ...
	  ?>	
		<table width=100%>
		<form name="adminForm"  action="<?php echo $PHP_SELF;?>" method="post">
                <tr><td>Page Title</td><td>Page Name</td></tr>
		<tr><td><input name="title" size=45 value="<?php  print $row['title'] ?>"></td><td><input name="namemenu" value="<?php  print $row['name'] ?>"></td></tr>
		<tr><td colspan=2>Page Text</td></tr>
	        <tr><td colspan=2><textarea name="content" cols=60 rows=10><?php  print $row['body']?></textarea></td></tr>
		<tr><td colspan=2 align=right><a href=../editor.htm  onClick="openBrWindow('../editor.htm'); return false";><?php  print TEXT_VISUAL_EDITOR ?></a></td></tr>
		<tr><td colspan=2><input type=submit value="Submit"></td></tr>		
		<input type=hidden name=act value="update">
		<input type=hidden name=id value="<?php  print $row['info_id'] ?>">
		</form>
		</table>
	  <?php
 	 }
